...
namespace Jde::Markets::BarData
{
	...
	Φ FindExisting( const Contract& contract, Day start=0, Day end=0, sv prefix={}, map<string,sp<Proto::BarFile>>* pPartials=nullptr )noexcept(false)->flat_set<Day>;
	...
	Φ TryLoad( const Contract& contract, Day start, Day end )noexcept->MapPtr<Day,VectorPtr<CandleStick>>;
	Φ Load( const Contract& contract, Day start, Day end )noexcept(false)->MapPtr<Day,VectorPtr<CandleStick>>;
	Φ CoLoad( ContractPtr_ contract, Day start, Day end )noexcept(false)->AWrapper;//map<Day,VectorPtr<CandleStick>>
	α Load( path path, sv symbol, const map<string,sp<Proto::BarFile>>* pPartials=nullptr )noexcept(false)->MapPtr<Day,VectorPtr<CandleStick>>;
	...
	Φ Save( const Contract& contract, const map<Day,VectorPtr<CandleStick>>& days, VectorPtr<tuple<TimePoint,optional<TimePoint>>> pExcluded=nullptr, bool checkExisting=false, const map<string,sp<Proto::BarFile>>* pPartials=nullptr )noexcept(false)->void;
